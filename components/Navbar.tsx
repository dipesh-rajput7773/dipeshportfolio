...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 bg-background ${
        scrolled ? 'py-4 border-b border-warm-white/5' : 'py-8'
      }`}
    >
        ...
        <div className="hidden md:flex items-center gap-12">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className={`mono text-[10px] font-bold uppercase tracking-[0.2em] transition-all relative group ${
                pathname === link.href ? 'text-warm-white' : 'text-muted hover:text-warm-white'
              }`}
            >
              {link.name}
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`mono text-[11px] font-bold uppercase tracking-[0.3em] ${
                  pathname === link.href ? 'text-crimson' : 'text-muted hover:text-warm-white'
                }`}
